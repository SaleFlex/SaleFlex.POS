    ...
    public partial class Dao
    {
        ...
        public List<CashReportDataModel> xGetCashReportDaily()
        {
            ...
            try
            {
                ...
                DataTable xDataTable = Dbo.xGetInstance(CommonProperty.prop_strDatabaseSalesFileNameAndPath).xExecuteDataTable(string.Format("SELECT ifnull(sum(h.ReceiptTotalPrice),0) as ReceiptTotal,(ifnull(sum(h.ReceiptTotalPrice),0) - ifnull(sum(h.ReceiptTotalVat),0)) as CashWithoutVat FROM TableTransactionHead h INNER JOIN TableTransactionDocumentType dt ON dt.Id=h.FkTransactionDocumentTypeId WHERE h.IsVoided=0 AND strftime('%d-%m-%Y',h.TransactionDateTime) = strftime('%d-%m-%Y','now') AND dt.[No] NOT IN (8,9,10)"));
                ...
                xDataTable = Dbo.xGetInstance(CommonProperty.prop_strDatabaseSalesFileNameAndPath).xExecuteDataTable(string.Format("SELECT ifnull(sum(h.ReceiptTotalPrice),0) as ReturnProductCash FROM TableTransactionHead h INNER JOIN TableTransactionDocumentType dt ON  dt.Id=h.FkTransactionDocumentTypeId WHERE h.IsVoided=0 AND strftime('%d-%m-%Y',h.TransactionDateTime) = strftime('%d-%m-%Y','now') AND dt.[No]=10"));
                ...
                xDataTable = Dbo.xGetInstance(CommonProperty.prop_strDatabaseSalesFileNameAndPath).xExecuteDataTable(string.Format("SELECT IFNULL(SUM(p.PaymentAmount),0) as CardPayment FROM TableTransactionPayment p INNER JOIN TableTransactionHead h ON h.Id=p.FkTransactionHeadId INNER JOIN TableTransactionDocumentType dt ON dt.Id=h.FkTransactionDocumentTypeId WHERE h.IsVoided=0 AND strftime('%d-%m-%Y',h.TransactionDateTime)= strftime('%d-%m-%Y','now') AND dt.[No] NOT IN (8,9,10) AND p.FkPaymentTypeId=2"));
                ...
                xDataTable = Dbo.xGetInstance(CommonProperty.prop_strDatabaseSalesFileNameAndPath).xExecuteDataTable(string.Format("SELECT IFNULL(SUM(p.PaymentAmount),0) as OtherPayment FROM TableTransactionPayment p INNER JOIN TableTransactionHead h ON h.Id=p.FkTransactionHeadId INNER JOIN TableTransactionDocumentType dt ON dt.Id=h.FkTransactionDocumentTypeId WHERE h.IsVoided=0 AND strftime('%d-%m-%Y',h.TransactionDateTime)= strftime('%d-%m-%Y','now') AND dt.[No] NOT IN (8,9,10) AND p.FkPaymentTypeId NOT IN (1,2)"));
